...
def encode_lossless_channel(channel_2d):
    ...
    if HAS_CPP:
        ...
        pr = np.where((pa <= pb) & (pa <= pc), left, np.where(pb <= pc, above, above_left))
        res3 = arr - pr
...
def encode_lossy(pixels, w, h, quality=5, preset="Balanced", channels=3, bit_depth=8, progressive=True, metadata=None):
    ...
    def process_channel_tile(tile_data, q_base, q_matrix_override=None):
        ...
        # kill the noise
        for b in [L1_HL, L1_LH, L1_HH]:
            b[np.abs(b) < noise_floor] = 0

        L2_LL_q = np.round(L2_LL).astype(np.int16)
        L2_HL_q, L2_LH_q, L2_HH_q = (
            np.round(L2_HL / q2).astype(np.int16),
            np.round(L2_LH / q2).astype(np.int16),
            np.round(L2_HH / q2).astype(np.int16),
        )
        L1_HL_q, L1_LH_q, L1_HH_q = (
            np.round(L1_HL / q1).astype(np.int16),
            np.round(L1_LH / q1).astype(np.int16),
            np.round(L1_HH / q1).astype(np.int16),
        )
    ...
    if use_mode_10:
            ...
            for tx in range(0, gw, tile_size):
                ...
                for c in range(channels):
                    if HAS_CPP:
                        ...
                        t_chan = np.zeros((tile_size, 16, tile_size, 16), dtype=np.float32)
                        tile_view = padded_chans[c].reshape(gh, 16, gw, 16)[
                            ty : ty + tile_size, :, tx : tx + tile_size, :
                        ]
                        t_chan[: tile_view.shape[0], :, : tile_view.shape[2], :] = tile_view

                    if not disable_ycocg and c == 0:
                        q_val, qm = quality, (q_matrix[0], q_matrix[1]) if q_matrix else None
                    elif not disable_ycocg and c < 3:
                        q_val, qm = max(1, quality - 1), (q_matrix[2], q_matrix[3]) if q_matrix else None
                    else:
                        q_val, qm = quality, (q_matrix[0], q_matrix[1]) if q_matrix else None
        ...
        for c in range(channels):
            if not disable_ycocg and c == 0:
                q_val, qm = quality, l_qm
            elif not disable_ycocg and c < 3:
                q_val, qm = max(1, quality - 1), c_qm
            else:
                q_val, qm = quality, l_qm
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            c0, c1, c2 = [
                f.result()
                for f in [executor.submit(lzma.compress, p, preset=lvl) for p in [l0_payload, l1_payload, l2_payload]]
            ]
...
def reconstruct_channel(b_list, mip_level=0):
    if mip_level >= 2:
        return b_list[0]  # quarter size (super fast)
    L1_LL = ihaar_level(b_list[0], b_list[1], b_list[2], b_list[3])
    if mip_level == 1:
        return L1_LL  # half size
    ...
    if mode == 10:
        ...
        def decode_tile(tx, ty):
            ...
            if eff_target >= 1:
                ...
                for c in range(channels):
                    [tile_bands[c].append(np.zeros((t_gh, t_gw, 4, 4), dtype=np.float32)) for _ in range(3)]
            ...
            if eff_target >= 2:
                ...
                for c in range(channels):
                    [tile_bands[c].append(np.zeros((t_gh, t_gw, 8, 8), dtype=np.float32)) for _ in range(3)]
            ...
            for c in range(channels):
                ...
                full_bands[c][ty * tile_size : ty * tile_size + rh, tx * tile_size : tx * tile_size + rw] = t_rec[
                    :rh, :rw
                ]
        ...
        with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
            futures = [
                executor.submit(decode_tile, tx, ty) for ty in range(tile_y1, tile_y2) for tx in range(tile_x1, tile_x2)
            ]
        ...
        for _ in range(3):
            ...
            offset += 4
            compressed_chunks.append(data[offset : offset + sz])
            offset += sz
        ...
        if target_layer >= 1:
            ...
            for c in range(channels):
                for _ in range(3):
                    bands[c].append(np.zeros((gh, gw, 4, 4), dtype=np.float32))
        ...
        if target_layer >= 2:
            ...
            for c in range(channels):
                for _ in range(3):
                    bands[c].append(np.zeros((gh, gw, 8, 8), dtype=np.float32))
