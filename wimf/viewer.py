    ...
    def __init__(self, root, filename):
        ...
        try:
            ...
            self.status_left = tk.Label(
                self.status_frame, text="", bg="#161616", fg="#bb86fc", font=("Segoe UI Bold", 9), padx=15
            )
            self.status_left.pack(side=tk.LEFT)

            hints = "SCROLL: ZOOM • DRAG: PAN"
            if self.is_animated:
                hints += " • SPACE: PLAY/PAUSE"
            self.status_right = tk.Label(
                self.status_frame, text=hints, bg="#161616", fg="#888888", font=("Segoe UI Bold", 8), padx=15
            )
            ...
            tk.Label(
                root, text=f"LOAD ERROR\n{e}", fg="#ff4444", bg="#0a0a0a", font=("Segoe UI Bold", 12), pady=50
            ).pack()
