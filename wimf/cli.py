...
def convert(
    ...
    try:
        if meta is None:
            meta = {"author": "Unknown", "engine": "WIMF Open Suite v19.0"}
        ...
        if in_ext in [".wimf", ".wif", ".awif"]:
            ...
            if depth_map_path:
                ...
                if is_10bit:
                    d_arr = d_arr.astype(np.uint16) * 4
            ...
            saveImage(
                output_path, w, h, p_bytes, compression=compression, quality=quality, metadata=meta, preset=preset
            )
...
def main():
    ...
    if args.cll:
        meta["max_cll"] = args.cll
    if args.fall:
        meta["max_fall"] = args.fall
