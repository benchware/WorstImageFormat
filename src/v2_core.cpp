...
std::vector<int64_t> wavelet_forward(const uint8_t* data,uint32_t w,uint32_t h,uint8_t bps,bool rev,unsigned levels,double q){
    ...
    std::vector<double> line(std::max(w,h)), e97(std::max(w,h)), o97(std::max(w,h));
    std::vector<int64_t> e53(std::max(w,h)), o53(std::max(w,h));
    for(unsigned level=0;level<levels;++level){for(uint32_t y=0;y<rh;++y){std::copy(a.begin()+y*w,a.begin()+y*w+rw,line.begin());line.resize(rw);if(rev)lift53_forward(line,e53,o53);else lift97_forward(line,e97,o97);std::copy(line.begin(),line.begin()+rw,a.begin()+y*w);}for(uint32_t x=0;x<rw;++x){for(uint32_t y=0;y<rh;++y)line[y]=a[y*w+x];line.resize(rh);if(rev)lift53_forward(line,e53,o53);else lift97_forward(line,e97,o97);for(uint32_t y=0;y<rh;++y)a[y*w+x]=line[y];}rw=(rw+1)/2;rh=(rh+1)/2;}
...
std::vector<uint8_t> wavelet_inverse(const int64_t* coeff,size_t count,uint32_t w,uint32_t h,uint8_t bps,bool rev,unsigned levels,double q){
    ...
    std::vector<double> line(std::max(w, h)), e97(std::max(w, h)), o97(std::max(w, h));
    std::vector<int64_t> e53(std::max(w, h)), o53(std::max(w, h));
    for(int level=static_cast<int>(levels)-1;level>=0;--level){const uint32_t rw=(w+(1u<<level)-1)>>level,rh=(h+(1u<<level)-1)>>level;for(uint32_t x=0;x<rw;++x){for(uint32_t y=0;y<rh;++y)line[y]=a[y*w+x];line.resize(rh);if(rev)lift53_inverse(line,e53,o53);else lift97_inverse(line,e97,o97);for(uint32_t y=0;y<rh;++y)a[y*w+x]=line[y];}for(uint32_t y=0;y<rh;++y){std::copy(a.begin()+y*w,a.begin()+y*w+rw,line.begin());line.resize(rw);if(rev)lift53_inverse(line,e53,o53);else lift97_inverse(line,e97,o97);std::copy(line.begin(),line.begin()+rw,a.begin()+y*w);}}
